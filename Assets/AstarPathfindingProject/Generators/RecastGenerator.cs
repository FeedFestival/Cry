...
//#define ASTAR_RECAST_LARGER_TILES //@SHOWINEDITOR Raises the vertex count limit on recast tiles, but reduces the max number of tiles.
//#define ASTAR_NO_JSON

#if UNITY_4_0 || UNITY_4_1 || UNITY_4_2 || UNITY_4_3 || UNITY_4_4 || UNITY_4_5 || UNITY_4_6 || UNITY_4_7 || UNITY_4_8 || UNITY_4_9 || UNITY_5_0 || UNITY_5_1 || UNITY_5_2 || UNITY_5_3 || UNITY_5_4 || UNITY_5_5 || UNITY_5_6 || UNITY_5_7
#define UNITY_GE_4
#endif
...
*/
	public class RecastGraph : NavGraph, INavmesh, IRaycastableGraph, IFunnelGraph, IUpdatableGraph, INavmeshHolder {
		...
		public void SnapForceBoundsToScene () {
			...
			GetSceneMeshes (forcedBounds, tagMask, mask, meshes);
		...
		public void GetRecastMeshObjs (Bounds bounds, List<ExtraMesh> buffer) {
			...
			// Create an ExtraMesh object for
			// each RecastMeshObj
					...
					if (rend.isPartOfStaticBatch) {
						...
						if (!staticMeshes.Contains (mesh)) {
							...
							for (int j=0;j<meshes.Count;j++) {
								if (meshes[j].original == mesh) { smesh = meshes[j]; break; }
							}
							smesh.bounds.Encapsulate (rend.bounds);
						}
	#endif
					} else {
		...
		public static bool IsJsEnabled () {
			if (Directory.Exists (Application.dataPath+"/AstarPathfindingEditor/Editor")) {
				return true;
			}
			return false;
		}
	...
	//#if (!UNITY_EDITOR && !UNITY_STANDALONE_OSX && !UNITY_STANDALONE_WIN) || UNITY_WEBPLAYER || UNITY_IPHONE || UNITY_ANDROID || UNITY_DASHBOARD_WIDGET || UNITY_XBOX360 || UNITY_PS3
		...
		void CollectColliderMeshes ( Bounds bounds, List<ExtraMesh> extraMeshes) {
			...
			for (int i = 0; i < colls.Length; i++) {
				...
				if (((1 << col.gameObject.layer) & mask) != 0 && col.enabled && !col.isTrigger && col.bounds.Intersects (bounds)) {
