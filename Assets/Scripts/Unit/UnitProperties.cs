...
public class UnitProperties : MonoBehaviour
{
    ...
    public void Initialize(Unit unit)
    {
        ...
        FeetCollider.transform.position = new Vector3(FeetCollider.transform.position.x,
            FeetCollider.transform.position.y + 0.2f,
            FeetCollider.transform.position.z);
        ...
        if (_unit.UnitType == UnitType.Player)
        {
            for (var i = 0; i < 4; i++)
            {
                BodyParts.Add(Logic.CreateFromPrefab("Prefabs/3DComponents/FeetCollider",
                            ThisUnitTransform.position));
            }
